...
$resultado= @mysql_query($sql) or die(mysql_error());
						...
						while ($datos = @mysql_fetch_assoc($resultado) ){
								...
								$confirmadoPor = $datos['confirmadoPor'];
						...
						while ($datos5 = @mysql_fetch_assoc($resultado5) ){
							...
							$sql6 = "SELECT * FROM cuponesgrupo WHERE NumeroHab = '$FolioCupon' AND FolioGrupoC = '$FolioGrupo'";

						$resultado6= @mysql_query($sql6) or die(mysql_error());

						while ($datos6 = @mysql_fetch_assoc($resultado6) ){

								$FolioGrupo = $datos["FolioGrupoC"];
...
$meses = array("Enero","Febrero","Marzo","Abril","Mayo","Junio","Julio","Agosto","Septiembre","Octubre","Noviembre","Diciembre");
					...
					$pdf->SetY(45);
					$pdf->SetX(20);		
					$pdf->Cell(100,8,utf8_decode("Hotel : ").utf8_decode($nombreHotel),0,0);
					$pdf->Cell(60,8,utf8_decode("Folio de Grupo: ").$_SESSION["FolioAux"],1,1);	
					$pdf->SetX(20);
					$pdf->MultiCell(160,8,utf8_decode("Direccion : ").utf8_decode($dirHotel),0,'L');						
					$pdf->SetX(20);
					...
					$pdf->SetX(20);		
					$pdf->Cell(160,10,utf8_decode("Pasajero Titular : ").utf8_decode($NombreTitular),'LR',1);					
					$pdf->SetX(20);		
					$pdf->Cell(160,10,utf8_decode("Segundo Pasajero : ").utf8_decode($NombreA),'LR',1);					
					$pdf->SetX(20);		
					$pdf->Cell(160,10,utf8_decode("Menores : ").utf8_decode($Menores)	,'LR',1);					
					$pdf->SetX(20);		
					...
					$pdf->Cell(80,10,utf8_decode("Clave Confirmacion : ").utf8_decode($claveConfirmacion),1,0,'L');
					$pdf->MultiCell(80,10,utf8_decode("Comfirmado por : ").utf8_decode($confirmadoPor),1,'L');					
					$pdf->SetX(20);
					$pdf->MultiCell(160,10,utf8_decode("Observaciones : ").utf8_decode($Observaciones),1,'L');					
					$pdf->SetX(20);
					$pdf->Cell(160,14,utf8_decode("Le atendio : ").utf8_decode($nombreU),1,'C');
					$pdf->SetY(225);
					...
					$pdf->Output('FolioG '.$NombreGrupo.' Hotel '.$nombreHotel.' PAX '.$NombreTitular.'.pdf','D');
